...
bool IRToBDI::convertNode(const IRNode& ir_node) { 
     ...
     switch (ir_node.opcode) { 
        ...
        case IROpCode::PARAM: // Represents function parameter input 
        ...
        case IROpCode::ALLOC_MEM: { 
         ...
         case IROpCode::STORE_MEM: { // Assignment 
             ...
             bdi_node_id = builder_.addNode(bdi_op, ir_node.label); 
             if (ir_node.inputs.size() != 2) return false; // Expect Addr, Value 
...
case IROpCode::CALL: bdi_op = BDIOperationType::CTRL_CALL; break; // Args connected, control connected later 
