...
void ExecutionContext::setNextArgument(PortIndex arg_index, BDIValueVariant value) {
 ...
 // Maybe get from global scope or error? For now, error.
        return std::nullopt;
    }
    const auto& current_frame = call_stack_.back();
    auto it = current_frame.arguments.find(arg_index);
    if (it != current_frame.arguments.end()) {
        return it->second;
    }
    return std::nullopt;
 }
 void ExecutionContext::setCurrentReturnValue(BDIValueVariant value) {
    if (call_stack_.empty()) {
        // Cannot set return value in global scope? Or maybe this is program exit value?
        last_return_value_ = std::move(value); // Set last return if no frame
        return;
    }
...
void ExecutionContext::pushCallFrame(NodeID return_node_id) {
    ...
    last_return_value_ = std::nullopt; // Clear previous return value
